...
export default function NoticeBoard() {
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 py-8">
      ...
      <p className="text-slate-400 dark:text-slate-400 text-sm mb-6">Community alerts, events, jobs and announcements — no ads here.</p>

      <div className="flex flex-wrap gap-2 mb-6">
        <input
          placeholder="Filter by area..."
          className="border border-slate-200 dark:border-white/10 rounded-lg px-3 py-2 text-sm w-40 bg-white dark:bg-[#18110c] text-slate-900 dark:text-white placeholder:text-slate-400 dark:placeholder:text-slate-500 outline-none focus:ring-2 focus:ring-accent-500/30 focus:border-accent-500 transition"
          value={area}
        ...
        <div className="flex gap-1.5 flex-wrap">
          <button
            ...
            className={`px-3 py-1.5 rounded-full text-xs font-medium transition ${
              category === '' ? 'bg-accent-600 text-white' : 'bg-slate-100 dark:bg-white/10 text-slate-600 dark:text-slate-300 hover:bg-slate-200 dark:hover:bg-white/15'
            }`}
          >
            All
          </button>
          {NOTICE_CATEGORIES.map((c) => (
            <button
              ...
              className={`px-3 py-1.5 rounded-full text-xs font-medium capitalize transition ${
                category === c ? 'bg-accent-600 text-white' : 'bg-slate-100 dark:bg-white/10 text-slate-600 dark:text-slate-300 hover:bg-slate-200 dark:hover:bg-white/15'
              }`}
            >
              {c}
            </button>
